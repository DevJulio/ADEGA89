...
function AddDrink(Arg) {
    ...
    if (Nome == "" || Tipo == "" || Pais == "" || Price == "") {
        ...
        db.collection('Loja').doc('Bebidas').collection('Cervejas').add({
            ...
            .then(function (docRef) {
                window.alert('Cerveja cadastrada!')
                SetDocId(docRef.id)
                getForm('Nome').value = '';
                getForm('Cup').value = '';
                getForm('Price').value = '';
...
function SetDocId(docId) {
    var gameRef = db.collection("Loja").doc("Bebidas").collection("Cervejas").doc(docId)
